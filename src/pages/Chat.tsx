  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 pb-24">
      ...
      <div className="card overflow-hidden flex" style={{ height: 'calc(100vh - 200px)', minHeight: '500px' }}>
        {/* Conversation list */}
        <div className={`w-full sm:w-80 border-r border-cyber/20 flex-shrink-0 flex flex-col ${showChat ? 'hidden sm:flex' : 'flex'}`}>
          <div className="p-4 border-b border-cyber/20">
            <h2 className="text-sm font-semibold text-heading">Диалоги</h2>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              Array.from({ length: 3 }).map((_, i) => <div key={i} className="p-4 animate-pulse"><div className="h-4 bg-cyber/10 rounded w-3/4 mb-1" /><div className="h-3 bg-cyber/10 rounded w-1/2" /></div>)
              ...
              <button
                ...
                className={`w-full p-4 flex items-center gap-3 hover:bg-cyber/5 transition-colors cursor-pointer border-b border-cyber/10 text-left ${activeConv?.id === conv.id ? 'bg-cyber/10' : ''}`}
              >
        ...
        <div className={`flex-1 flex flex-col ${showChat ? 'flex' : 'hidden sm:flex'}`}>
          {activeConv ? (
            <>
              <div className="p-4 border-b border-cyber/20 flex items-center gap-3">
                <button onClick={() => setActiveConv(null)} className="sm:hidden text-muted hover:text-[#00f5ff] cursor-pointer"><ArrowLeft size={18} /></button>
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : ''}`}>
                      <div className={`max-w-[70%]`}>
                        <div className={`inline-block px-4 py-2.5 rounded-2xl text-sm ${isMe ? 'bg-cyber/15 text-heading' : 'bg-nebula border border-cyber/20 text-body'}`}>
                          {msg.content}
                        </div>
              ...
              <div className="p-4 border-t border-cyber/20">
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-cyber/10 border border-cyber/30 rounded-xl px-4 py-3 text-sm text-heading placeholder:text-muted focus:outline-none focus:border-cyber"
                  />
                  <button onClick={handleSend} disabled={sending || !newMsg.trim()} className="px-4 py-2 bg-cyber/20 text-[#00f5ff] rounded-xl hover:bg-cyber/30 transition-all cursor-pointer disabled:opacity-50">
                    <Send size={18} />
                  </button>
                </div>
              </div>
