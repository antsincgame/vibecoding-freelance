...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
  ...
  waiting: { label: 'Ожидает', color: 'text-[#00f5ff]', icon: Clock },
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex gap-2">
            {selectedTicket.status !== 'closed' ? (
              <button onClick={() => handleClose(selectedTicket.id)} className="px-4 py-2 text-sm bg-[#00f5ff]/10 text-muted rounded-lg hover:text-body cursor-pointer">Закрыть</button>
            ) : (
              <button onClick={() => handleReopen(selectedTicket.id)} className="px-4 py-2 text-sm bg-[#00f5ff]/10 text-[#00f5ff] rounded-lg hover:bg-[#00f5ff]/20 cursor-pointer">Открыть</button>
            )}
          </div>
        </div>
    ...
    <AdminLayout>
      ...
      <div className="space-y-2">
        {loading ? Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="card p-5 animate-pulse"><div className="h-4 bg-[#00f5ff]/10 rounded w-1/2 mb-2" /><div className="h-3 bg-[#00f5ff]/10 rounded w-1/3" /></div>
        )) : tickets.map((ticket) => {
          const st = STATUS_MAP[ticket.status] || STATUS_MAP.open;
          const StIcon = st.icon;
          return (
            <div key={ticket.id} onClick={() => openTicket(ticket)} className={`card p-5 cursor-pointer hover:border-[#00f5ff]/30 transition-all ${ticket.status === 'open' ? 'border-neon-green/30' : ''}`}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted bg-[#00f5ff]/10 px-2 py-0.5 rounded-full">{CAT_MAP[ticket.category] || ticket.category}</span>
                  </div>
