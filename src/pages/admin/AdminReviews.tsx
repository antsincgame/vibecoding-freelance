    ...
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3 text-sm text-muted">
          ...
          <span className="flex items-center gap-1"><Star size={14} className="text-gold fill-gold" /> {avgRating}</span>
        </div>
      </div>

      <div className="space-y-3">
        {loading ? Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="card p-5 animate-pulse"><div className="h-4 bg-gold/10 rounded w-1/3 mb-2" /><div className="h-3 bg-gold/10 rounded w-2/3" /></div>
        )) : reviews.map((review) => (
            ...
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0">
                {review.author_avatar ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-gold/20 flex items-center justify-center text-sm text-gold">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <div className="flex gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} size={12} className={i < (review.rating || 0) ? 'fill-gold text-gold' : 'text-muted/30'} />
                    ))}
                  </div>
                ...
                {review.reply && (
                  <div className="pl-4 border-l-2 border-gold/30 mt-2">
                    <p className="text-xs text-gold mb-0.5">Ответ фрилансера</p>
              ...
              <button onClick={() => handleDelete(review.id)} className="p-2 text-muted hover:text-neon-rose hover:bg-gold/10 rounded-lg cursor-pointer flex-shrink-0" title="Удалить">
                <Trash2 size={16} />
              </button>
            </div>
          </div>
