        ...
        <div className="flex-1 min-w-0 space-y-8">
          <div className="space-y-3">
            <div className="relative aspect-video rounded-2xl overflow-hidden bg-nebula">
              ...
              {images.length > 1 && (<>
                <button onClick={prevImage} className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-void/80 backdrop-blur-sm border border-cyber/20 flex items-center justify-center text-heading hover:bg-void transition-colors cursor-pointer"><ChevronLeft size={20} /></button>
                <button onClick={nextImage} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-void/80 backdrop-blur-sm border border-cyber/20 flex items-center justify-center text-heading hover:bg-void transition-colors cursor-pointer"><ChevronRight size={20} /></button>
              </>)}
              {user && (
                <button onClick={toggleFavorite} className={`absolute top-3 right-3 w-10 h-10 rounded-full bg-void/80 backdrop-blur-sm border border-cyber/20 flex items-center justify-center cursor-pointer transition-all ${isFavorite ? 'text-neon-rose' : 'text-muted hover:text-neon-rose'}`}>
            ...
            {images.length > 1 && (
              <div className="flex gap-2">
                {images.map((img, i) => (<button key={i} onClick={() => setCurrentImage(i)} className={`w-20 h-14 rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${i === currentImage ? 'border-cyber' : 'border-cyber/20 opacity-60 hover:opacity-100'}`}><img src={img} alt="" className="w-full h-full object-cover" /></button>))}
              </div>
            )}
          </div>
          <div>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr className="border-b border-cyber/20"><th className="text-left py-3 px-4 text-sm text-muted font-medium">{t('gig.package')}</th>
                  {(Object.keys(gig.packages) as PackageKey[]).map((key) => (<th key={key} className="text-center py-3 px-4"><span className={`text-sm font-semibold ${key === activePackage ? 'text-[#00f5ff]' : 'text-heading'}`}>{packageLabels[key]}</span></th>))}
                </tr></thead>
                <tbody>
                  <tr className="border-b border-cyber/20"><td className="py-3 px-4 text-sm text-muted">{t('gig.price')}</td>
                    {(Object.keys(gig.packages) as PackageKey[]).map((key) => (<td key={key} className="text-center py-3 px-4 font-mono text-sm font-bold text-[#00f5ff]">{gig.packages[key].price.toLocaleString('ru-RU')} ₽</td>))}
                  </tr>
                  <tr className="border-b border-cyber/20"><td className="py-3 px-4 text-sm text-muted">{t('gig.deadline')}</td>
                  ...
                  {gig.packages.premium.features.map((_, fi) => (
                    <tr key={fi} className="border-b border-cyber/30"><td className="py-3 px-4 text-sm text-muted">{gig.packages.premium.features[fi]}</td>
          ...
          <div className="flex flex-wrap gap-2">
            {gig.tags.map((tag) => <span key={tag} className="px-3 py-1.5 text-xs bg-cyber/10 text-muted rounded-lg border border-cyber/20">{tag}</span>)}
          </div>
          <div>
            ...
            <div className="space-y-6">
              {(reviews || []).map((review) => (
                <div key={review.id} className="space-y-3">
                  ...
                  {review.reply && (<div className="ml-11 pl-4 border-l-2 border-cyber/30"><p className="text-xs text-[#00f5ff] mb-1">{t('gig.freelancer_reply')}</p><p className="text-sm text-muted">{review.reply}</p></div>)}
                </div>
              ))}
        ...
        <div className="lg:w-[380px] flex-shrink-0">
          <div className="lg:sticky lg:top-24 space-y-6">
            <div className="card p-6 space-y-5">
              <div className="flex rounded-xl overflow-hidden border border-cyber/30">
                {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                  <button key={key} onClick={() => setActivePackage(key)} className={`flex-1 py-3 text-sm font-medium text-center transition-all cursor-pointer ${activePackage === key ? 'bg-cyber/10 text-[#00f5ff] border-b-2 border-cyber' : 'text-muted hover:text-body'}`}>{packageLabels[key]}</button>
                ))}
              </div>
              <div className="space-y-3">
                <div className="flex items-center justify-between"><span className="text-body text-sm">{pkg.description}</span><span className="text-2xl font-bold text-[#00f5ff] font-mono">{pkg.price.toLocaleString('ru-RU')} ₽</span></div>
              ...
              {showOrderForm && (
                <div className="space-y-3 pt-2">
                  <div className="border-t border-cyber/20 pt-3">
                    <p className="text-xs text-[#00f5ff] mb-2">Опишите ваши требования:</p>
                    <textarea
                      rows={3}
              ...
              {extrasTotal > 0 && (
                <div className="flex justify-between text-sm pt-2 border-t border-cyber/20">
                  <span className="text-muted">Итого с опциями:</span>
                  <span className="text-[#00f5ff] font-mono font-bold">{(pkg.price + extrasTotal).toLocaleString('ru-RU')} ₽</span>
                </div>
              )}
            ...
            <div className="card p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="bg-cyber/10 rounded-xl p-3 border border-cyber/30"><p className="text-lg font-bold text-heading font-mono">{gig.freelancer.ordersCompleted}</p><p className="text-xs text-muted">{t('gig.completed_orders')}</p></div>
                <div className="bg-cyber/10 rounded-xl p-3 border border-cyber/30"><p className="text-lg font-bold text-neon-emerald font-mono">{gig.freelancer.successRate}%</p><p className="text-xs text-muted">{t('gig.success_rate')}</p></div>
              </div>
