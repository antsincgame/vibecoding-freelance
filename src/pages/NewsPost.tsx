...
export default function NewsPost() {
  ...
  if (!post) return (
    <div className="max-w-3xl mx-auto px-4 py-20 text-center">
      ...
      <Link to="/news" className="text-[#00f5ff] text-sm mt-4 inline-block">← Все новости</Link>
    </div>
  );
  ...
  const renderContent = (text: string) => {
    return text.split('\n').map((line, i) => {
      ...
      if (line.startsWith('- ')) return <div key={i} className="flex gap-2 ml-2 mb-1"><span className="text-[#00f5ff]">•</span><span>{renderInline(line.slice(2))}</span></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 pb-24">
      ...
      <Link to="/news" className="flex items-center gap-2 text-sm text-muted hover:text-[#00f5ff] mb-8">
      ...
      {/* Share */}
      <div className="flex items-center gap-4 mt-10 pt-6 border-t border-[#00f5ff]/20">
        <button
          onClick={() => navigator.clipboard.writeText(window.location.href)}
          className="flex items-center gap-2 px-4 py-2 bg-[#00f5ff]/10 text-[#00f5ff] rounded-xl text-sm hover:bg-[#00f5ff]/20 cursor-pointer"
        >
      ...
      <div className="flex items-stretch gap-4 mt-8">
        {prevPost ? (
          <Link to={`/news/${prevPost.id}`} className="flex-1 card p-4 hover:border-[#00f5ff]/30 transition-all group">
            <p className="text-[10px] text-muted mb-1">← Предыдущая</p>
            <p className="text-sm font-medium text-heading group-hover:text-[#00f5ff] transition-colors line-clamp-2">{prevPost.title}</p>
          </Link>
        ) : <div className="flex-1" />}
        {nextPost ? (
          <Link to={`/news/${nextPost.id}`} className="flex-1 card p-4 hover:border-[#00f5ff]/30 transition-all group text-right">
            <p className="text-[10px] text-muted mb-1">Следующая →</p>
            <p className="text-sm font-medium text-heading group-hover:text-[#00f5ff] transition-colors line-clamp-2">{nextPost.title}</p>
          </Link>
        ) : <div className="flex-1" />}
      </div>
    </div>
