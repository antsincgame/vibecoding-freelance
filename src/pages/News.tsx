    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 pb-24">
      ...
      <div className="space-y-4">
        {NEWS_DATA.map((item) => {
          ...
          return (
            <Link key={item.id} to={`/news/${item.id}`} className="card p-6 flex items-start gap-5 hover:border-gold/30 transition-all group block">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h2 className="text-base font-heading font-semibold text-heading mb-1 group-hover:text-gold transition-colors">{item.title}</h2>
                <p className="text-sm text-muted leading-relaxed">{item.summary}</p>
              </div>
              <ArrowRight size={18} className="text-muted group-hover:text-gold flex-shrink-0 mt-3 transition-colors" />
            </Link>
          );
        })}
      </div>
